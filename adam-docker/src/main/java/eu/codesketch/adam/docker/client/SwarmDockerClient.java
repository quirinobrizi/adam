...
import eu.codesketch.adam.docker.command.NodeCommandImpl;
import eu.codesketch.adam.docker.command.StatisticsCommand;
import eu.codesketch.adam.docker.command.StatisticsCommandImpl;
 ...
 *
 */
public class SwarmDockerClient implements Cloneable, DockerClient, SwarmClient {

    private final DockerClientConfig dockerClientConfig;

    private SwarmDockerExecFactory dockerCmdExecFactory;

    private SwarmDockerClient() {
        this(DefaultDockerClientConfig.createDefaultConfigBuilder().build());
    }

    private SwarmDockerClient(String serverUrl) {
        this(configWithServerUrl(serverUrl));
    }

    private SwarmDockerClient(DockerClientConfig dockerClientConfig) {
    ...
    public static SwarmDockerClient getInstance() {
        return new SwarmDockerClient();
    }

    public static SwarmDockerClient getInstance(DockerClientConfig dockerClientConfig) {
        return new SwarmDockerClient(dockerClientConfig);
    }

    public static SwarmDockerClient getInstance(String serverUrl) {
        return new SwarmDockerClient(serverUrl);
    }

    public SwarmDockerClient withDockerCmdExecFactory(SwarmDockerExecFactory dockerCmdExecFactory) {
    ...
    private SwarmDockerExecFactory getDockerCmdExecFactory() {
        checkNotNull(dockerCmdExecFactory, "dockerCmdExecFactory was not specified");
    ...
    @Override
    public StatisticsCommand statisticsCommand(String containerId) {
        return new StatisticsCommandImpl(getDockerCmdExecFactory().createStatisticsCommandExec(), containerId);
    }
    ...
    @Override
    public NodeCommand nodeCommand() {
        return new NodeCommandImpl(getDockerCmdExecFactory().createNodeCommandExec());
    }
